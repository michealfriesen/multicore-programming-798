...
static volatile char padding0[PADDING_BYTES];
static cpu_set_t *cpusets[MAX_THREADS];
static int customBinding[MAX_THREADS];
static int numCustomBindings = 0;
static int numLogicalProcessors = MAX_THREADS;
static volatile char padding1[PADDING_BYTES];
...
static unsigned parseToken(std::string argv, int ix) {
    ...
    if (ix >= argv.size() || argv[ix] == ',') {
        ...
        assert(numCustomBindings <= MAX_THREADS);
