...
//    PAD;
#define PADDING_INT_FACTOR (128/sizeof(int))
    int init[MAX_THREADS_POW2*PADDING_INT_FACTOR] = {0,};
...
//    PAD;
    ...
    void initThread(const int tid) {
        if (init[tid*PADDING_INT_FACTOR]) return; else init[tid*PADDING_INT_FACTOR] = !init[tid*PADDING_INT_FACTOR];
...
//            std::cout<<"setting quiescent state for just one record type: "<<typeid(RecordTypesFirst).name()<<"\n";
    ...
    inline void startOp(const int tid, const bool readOnly = false) {
        if (!init[tid*PADDING_INT_FACTOR]) {
            initThread(tid);
        }
...
class simple_record_manager : public record_manager<reclaimer_debra<>, allocator_new<>, pool_none<>, RecordTypes...> {
public:
    simple_record_manager(const int numProcesses, const int _neutralizeSignal = -1 /* unused here */)
            : record_manager<reclaimer_debra<>, allocator_new<>, pool_none<>, RecordTypes...>(numProcesses, _neutralizeSignal) {}
};
